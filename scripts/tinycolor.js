...
function generate (scheme){
    ...
    const generator = new tinycolor(document.getElementById("baseGenerateColor").value);
    const outputColors = new Array(5);
    for(i=0;i<5;i++){
      outputColors[i] = -1;
    }
    var temp;
    ...
    if(outputColors[0] == -1){
      outputColors[0] = "#FF0000";
    }
    if(outputColors[1] == -1){
      outputColors[1] = "#00FF00";
    }
    if(outputColors[2] == -1){
      outputColors[2] = "#0000FF";
    }
    if(outputColors[3] == -1){
    ...
    for (i = 0; i < palettes.length; i++)
  {
    ...
    {
        palettes[i].value = outputColors[i];
    }
  }
    const event = new Event('change', {"bubbles": true});
    palettes[0].dispatchEvent(event);
}
(function(Math) {
